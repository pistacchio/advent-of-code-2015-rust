...
// (hp, damage, armor)
const BOSS: (i32, i32, i32) = (100, 8, 2);
...
fn find_optimal_equipment() -> (i32, i32) {
        ...
        for a in -1..(ARMORS.len() as i32) {
            ...
            for num_rings in 0..=2 {
                let ring_combinations: Vec<Vec<(i32, i32, i32)>> = (0..RINGS.len())
                    ...
                    .map(|c| {
                        c.into_iter().map(|ridx| RINGS[ridx]).collect()
                    })
                    .collect();
                ...
                for rings in ring_combinations {
                    ...
                    if player_wins_fight(PLAYER_HP, player_damage, player_armor, BOSS.0, BOSS.1, BOSS.2) {
...
pub fn run() -> String {
    find_optimal_equipment().0.to_string()
}

pub fn run_pt2() -> String {
    find_optimal_equipment().1.to_string()
}
